...
describe('THE BEAUTY API / Router de carts', () => {
	let adminCookie;
	...
	it('POST /api/carts : Debe crear un carrito correctamente', async function () {
		...
		expect(createCartResponse.body.payload.payload.message).to.equal(
			'Nuevo carrito creado'
		);
		expect(typeof cartId).to.equal('string');
		expect(createCartResponse.body.payload.payload.data.products).to.be.an(
			'array'
		).that.is.empty;
	...
	it('GET /api/carts/:cid: Debe devolver un array vacío de productos obtenido del carrito por su id', async function () {
		...
		expect(getCartResponse.body.payload.payload.message).to.equal(
			'Productos del carrito obtenidos correctamente'
		);
		expect(typeof cartId).to.equal('string');
		expect(getCartResponse.body.payload.payload.data).to.be.an('array').that.is
			.empty;

		req.logger.test(`GET /api/carts/:cid ~ Cart get ~ cid: ${cartId}`);
		req.logger.test(
			`GET /api/carts/:cid ~ Cart get ~ array: ${JSON.stringify(
				getCartResponse.body.payload.payload.data,
				null,
				2
			)}`
		);
	...
	it('DELETE /api/carts/:cid: Debe eliminar el carrito por su id', async function () {
		...
		expect(deleteCartResponse.body.payload.message).to.equal(
