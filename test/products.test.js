...
describe('THE BEAUTY API / Router de products', () => {
	let adminCookie;
	...
	it('POST /api/products: Debe crear un producto correctamente', async function () {
		...
		expect(createProductResponse.body.payload.message).to.equal(
			'Producto agregado correctamente'
		);
		expect(typeof productIdToDelete).to.equal('string');
		expect(createProductResponse.body.payload.payload.title).to.equal(
			newProduct.title
		);
		expect(createProductResponse.body.payload.payload.description).to.equal(
			newProduct.description
		);

		req.logger.test(
			`POST /api/products ~ Product add ~ pid: ${productIdToDelete}`
		);
	});
	...
	it('GET /api/products: Debe obtener todos los productos', async function () {
		const getAllProductsResponse = await request(app)
			.get('/api/products')
			.set('Cookie', `jwt=${adminCookie}`);

		expect(getAllProductsResponse.status).to.equal(200);
		expect(getAllProductsResponse.body.success).to.be.true;
		expect(getAllProductsResponse.body.payload.payload.payload).to.be.an(
			'array'
		);

		req.logger.test(
			`GET /api/products ~ Products get ~ success: ${getAllProductsResponse.body.success}`
		);
		req.logger.test(
			`GET /api/products ~ Products get ~ object: ${getAllProductsResponse.body.payload.payload}`
		);
	});
	...
	it('DELETE /api/products/:pid: Debe eliminar un producto correctamente', async function () {
		...
		expect(deleteProductResponse.body.payload.message).to.equal(
		...
		req.logger.test(
			`DELETE /api/products ~ Product delete ~ pid: ${productIdToDelete}`
		);
	});
