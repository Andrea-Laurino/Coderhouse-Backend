...
describe('THE BEAUTY API / Router de sessions', () => {
	const randomEmailOnlyRegister = `user${Math.floor(
		Math.random() * 10000
	)}@example.com`;
    ...
    asincronía si demora en guardarse en la base de datos */
		...
		userCookie = loginResponse.headers['set-cookie'][0]
			.split(';')[0]
			.split('=')[1];
	});
	...
	it('POST /api/session/auth/register: Debe crear un usuario correctamente', async function () {
		...
		expect(response.body.payload.payload.message).to.equal(
			'Usuario agregado correctamente'
		);
		expect(response.body.payload.payload.data.email).to.equal(
			randomEmailOnlyRegister
		);

		req.logger.test(
			`POST /api/session/auth/register ~ User register ~ email: ${response.body.payload.payload.data.email}`
		);
	});
	...
	it('POST /api/session/auth/login: Debe loguear a un usuario correctamente y devolver una cookie', async function () {
		...
		req.logger.test(
			`POST /api/session/auth/login ~ User login ~ email : ${randomEmail}`
		);
		req.logger.test(
			`POST /api/session/auth/login ~ User login ~ cookie : ${JSON.stringify(
				cookie.name,
				null,
				2
			)}`
	...
	it('GET /api/session/auth/current: Debe enviar la cookie que contiene el usuario y desestructurarlo correctamente', async function () {
		const response = await request(app)
			.get('/api/session/auth/current')
			.set('Cookie', `jwt=${userCookie}`);
