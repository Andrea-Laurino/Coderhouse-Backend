...
class CartsServices {
	...
	initializeCartCollection = async () => {
		try {
			...
			console.error(
				'Error al inicializar la colección de carritos en la base de datos',
				error
			);
		}
	};
	...
	deleteAllProductsFromCart = async (cid, res) => {
		try {
			/* Repository */
			const cart = await cartsServices.findByIdAndUpdate(
				cid,
				{ $set: { products: [] } },
				{ new: true }
			);
	...
	purchaseCart = async (cid, req, res) => {
		...
		try {
			...
			cart.products = productsToPurchase.filter((productData) =>
				productsNotPurchased.includes(productData.productId)
			);
			...
			if (productsNotPurchased.length === 0) {
				...
				return res.sendSuccess({
					message:
						'Compra parcial. Algunos productos no pudieron ser comprados.',
					payload: {
