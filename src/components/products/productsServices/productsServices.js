...
} = require('../../../utils/errors/services/info');
/* ************************************************************************** */
class ProductsServices {
	getAllProducts = async (limit, page, sort, query, res) => {
		try {
			...
			if (page && !/^\d+$/.test(page)) {
				...
				page &&
				(parseInt(page) < 1 || parseInt(page) > result.totalPages)
			) {
				...
				const data = {
					...
					prevLink: result.hasPrevPage
						? `/products?limit=${options.limit}&page=${result.prevPage}&sort=${sort}&query=${query}`
						: null,
					nextLink: result.hasNextPage
						? `/products?limit=${options.limit}&page=${result.nextPage}&sort=${sort}&query=${query}`
						: null,
				};
	...
	updateProduct = async (pid, updateFields, res, req) => {
		try {
			...
			if (invalidFields.length > 0) {
				return res.sendUserError(
					`Los siguientes campos no se pueden modificar: ${invalidFields.join(
						', '
					)}`
				);
			} else {
	...
	getProducts = async (limit, page, sort, query, res) => {
		try {
			...
			const prevLink = result.hasPrevPage
				? `/products?limit=${options.limit}&page=${result.prevPage}`
				: null;
			const nextLink = result.hasNextPage
				? `/products?limit=${options.limit}&page=${result.nextPage}`
				: null;
	...
	getAdminProducts = async (limit, page, sort, query, res) => {
		try {
			...
			const nextPage = result.hasNextPage ? result.nextPage : null;
			const prevLink = result.hasPrevPage
				? `/admin/products?limit=${options.limit}&page=${result.prevPage}`
				: null;
			const nextLink = result.hasNextPage
				? `/admin/products?limit=${options.limit}&page=${result.nextPage}`
				: null;
	...
	getRealTimeProducts = async (limit, page, sort, query, res) => {
		try {
			...
			const nextPage = result.hasNextPage ? result.nextPage : null;
			const prevLink = result.hasPrevPage
				? `/realtimeproducts?limit=${options.limit}&page=${result.prevPage}`
				: null;
			const nextLink = result.hasNextPage
				? `/realtimeproducts?limit=${options.limit}&page=${result.nextPage}`
				: null;
	...
	getHomeProducts = async (limit, page, sort, query, res) => {
		try {
			...
			const nextPage = result.hasNextPage ? result.nextPage : null;
			const prevLink = result.hasPrevPage
				? `/home?limit=${options.limit}&page=${result.prevPage}`
				: null;
			const nextLink = result.hasNextPage
				? `/home?limit=${options.limit}&page=${result.nextPage}`
				: null;
