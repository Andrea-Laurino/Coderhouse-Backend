...
class UsersServices {
	...
	recoveryUser = async ({ email, password, res }) => {
		try {
			...
			/* Repository */
			let data = await usersServices.findByIdAndUpdate(
				user._id,
				{ password: createHash(password) },
				{ new: true }
			);
	...
	resetPass = async ({ email, password, res, req }) => {
		try {
			...
			if (passwordMatch) {
				...
				return res.sendUserError(
					'La nueva contraseña es la misma que la contraseña actual. No se puede colacar la misma contraseña.'
				);
			}
    ...
    `;
			...
			req.logger.info(
				'Mail de reestablecimiento de contraseña enviado correctamente'
			);
			return res.sendSuccess({
				payload: {
					message:
			...
			req.logger.error(
				'Error al reestablecer la contraseña y enviar el correo electrónico'
			);
	...
	updateUserPremium = async (uid, updateFields, res, req) => {
		try {
			...
			if (
				...
				!['user', 'premium'].includes(updateFields.role)
			) {
				return res.sendUserError(
			...
			return res.sendSuccess({
				message: 'Role de user actualizado correctamente',
				payload: data,
			});
		} catch (error) {
			return res.sendServerError('Error al actualizar el usuario');
